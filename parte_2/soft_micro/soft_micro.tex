...
La única función definida en este archivo es \mintinline[style=arduino,escapeinside=||] {C++}
{|\textcolor{orange}{leer\_encoders()}|} es devolver el valor leido por el conversor analógico digital, cuando se esta autocalibrando. Esta función se muestra en el código \ref{cod:lectura_encoders.cpp}. Si no se esta autocalibrando, deberá devolver las coordenadas correspondientes. En este caso, esta parte, se va programar en la próxima sección. 
...
Las ecuaciones mostradas anterioremente, se implementan dentro de la función \mintinline{c++}{leer_encoders.} 
...
Además, se revisa la conexión mediante el software wireshark. Los resultados del análisis se muestran a continuación: 
...
Luego de realizar esto último, se obtiene la dirección IP mediante DHCP , y se programa la conexión dentro del bucle principal del programa. A continuación se muestra la parte de inicialización, y el bucle principal del programa, donde el código obtenido hasta el momento, se muestra en el apéndice del presente capítulo 
